...
3.  **Estimate Calories**: You MUST provide an \`estimatedCalories\` value. If the image is not food, return 0.
...
const analyzeFoodImageFlow = ai.defineFlow(
  ...
  async (input) => {
    try {
      ...
      console.error('[NutriSnap] Error in analyzeFoodImageFlow:', e);
      throw new Error(
